    ...
    def handle_events(self, events, **kwargs):
        ...
        for event in events:
            if event.type == pygame.MOUSEBUTTONDOWN and self.rect.collidepoint(*mouse):
                ...
                self.held = True
                Sprite.add(self, kwargs['groups'][0])
                Sprite.remove(self, kwargs["groups"][1])
                ...
                self.held = False
                if kwargs["panel"].rect.collidepoint(*mouse):
                    self.kill()
                else:
                    Sprite.add(self, kwargs["groups"][1])
                    Sprite.remove(self, kwargs['groups'][0])
...
class Shadow(Sprite):
    ...
    def __init__(self, thing: Thing):
        ...
        # self.image = pygame.transform.scale(pygame.image.load("images/shadow.png").convert_alpha(), thing.image.get_size())
        self.image = self.thing.image.copy()
        self.image.fill((255, 255, 255, 78), None, pygame.BLEND_RGBA_MULT)
        self.rect = pygame.Rect(*thing.rect.topleft, *thing.rect.size)
        self.rect.center = (thing.rect.centerx + 10, thing.rect.centery + 10)
