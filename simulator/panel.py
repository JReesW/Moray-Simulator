    ...
    def handle_events(self, events):
        ...
        for event in events:
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                for i, (name, comp_type) in enumerate(self.components.items()):
                    comp = comp_type(self.scene.grid.tile_size)
                    if pygame.Rect(20, 20 + (comp.rect.h + 30) * i, *comp.rect.size).collidepoint(*mouse):
                        self.scene.add_component(comp_type(self.scene.grid.tile_size))
